import requests
from bs4 import BeautifulSoup

f = open('jiomart_scraped_data.csv','w')
f.write('Company Name,Item Name,Price,Image \n')

def nextpage():
    for i in range(1,27):   

        url = 'https://www.jiomart.com/c/groceries/2?prod_mart_groceries_products_popularity%5Bpage%5D='

        only_url = url.split('=')[-2]

        urls = only_url+'='+str(i)
        print(urls)
        response = requests.get(urls)
        soup = BeautifulSoup(response.text, 'html.parser')
        containers = soup.find_all('div', class_='col-md-3 p-0')

        for i in containers:
            name = i.find('span', class_='clsgetname').text
            price = i.find('span', id='final_price').text
            # image = i.find('img', id='product-image-photo lazyautosizes lazyloaded')
            image = i.find('img')['data-src']
            print(name)


            # print(f' Jiomart, {name},{price}, {image}\n')
            f.write(f' Jiomart, {name},{price}, {image}\n')
        print('\n')
   
     
nextpage()


f.close()
